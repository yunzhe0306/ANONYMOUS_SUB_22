...
class Logger(object):
    def __init__(self, stdout):
        ...
        self.log = open("./Neural-UCB-logs/logfile_" + dt_string + "_Neural-Thompson_Sampling.log", "w")
        self.out = stdout
...
def flatten(tensor):
    ...
    for element in tensor:
        T = torch.cat([T, element.cuda().flatten()])
    return T
...
class NeuralTSDiag:
    ...
    def init_kernel_matrix(self, A):
        ...
        for c_i in range(A):
            ...
            g = torch.cat([p.grad.flatten().detach() for p in self.func.parameters()])
            g = g / (np.sqrt(self.m))
            self.U += torch.matmul(g, g.T).cuda()
    ...
    def select(self, context, t):
        ...
        for i, fx in enumerate(mu):
            ...
            sample = torch.normal(mean=fx.item(), std=sigma)

            if (t+1) % 20 == 0:
                print("Arm: {}, f_hat: {}, UCB: {}".format(i, fx.item(), sample))
...
if __name__ == '__main__':
    ...
    #
    n_UCB = NeuralTSDiag(matrix_dim, args.lamdba, args.nu, hidden=args.hidden, sample_num=sample_num)

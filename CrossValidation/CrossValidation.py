...
def CrossValidRegression(bw_x_grid, lambda_reg_grid, bw_prod_grid, bw_prob_grid, fold_cv, algorithm_flag,
                         data_flag, data_flag_multiclass, A, N_valid, N, T, randomSeeds, RunNumber,
                         Main_Program_flag):
    # It is assumed that A is a multiple of 5 and
    DataXY = DC.TrainDataCollect(data_flag, data_flag_multiclass, A, N_valid, N, T, randomSeeds[0], RunNumber,
                                 Main_Program_flag)
    A = DataXY['NoOfArms']
    theta = DataXY['theta']
    ...
    # kf = KFold(samples_per_task[0], n_folds=fold_cv) --- Deprecated
    kf = KFold(n_splits=fold_cv)
    err = np.zeros([bw_x_grid.shape[0], bw_prod_grid.shape[0], bw_prob_grid.shape[0], lambda_reg_grid.shape[0]])

    print("Running Cross-validation...")
    print("Total: ", bw_x_grid.shape[0], bw_prod_grid.shape[0], bw_prob_grid.shape[0], lambda_reg_grid.shape[0])
        ...
        for bb3 in range(0, bw_prod_grid.shape[0]):
            ...
            for bb2 in range(0, bw_prob_grid.shape[0]):
                ...
                for ll in range(0, lambda_reg_grid.shape[0]):
                    ...
                    for train_index, test_index in kf.split(range(samples_per_task[0])):
                        ...
                        for ii in range(0, A):
                            ind_test[ii * test_index.shape[0]:(ii + 1) * test_index.shape[0]] = \
                                ind_all[ii * samples_per_task[ii]:(test_index.shape[0] + ii * samples_per_task[ii])]
                        ...
                        K_train, Task_sim = GK.GetKernelMatrixWithoutCorrectionTerm(X_train, A, X_train.shape[0],
                                                                                    samples_per_task_train,
                                                                                    bw_x, bw_prob, bw_prod,
                                                                                    algorithm_flag, theta)

                        alpha_est = alphaEst(X_train, lambda_reg, Y_train, samples_per_task_train, K_train, A)

                        K_test = GK.GetTestKernelMatrixWithoutCorrectionTerm(X_train, X_test, A,
                                                                             Task_sim, samples_per_task_train,
                                                                             samples_per_task_test, bw_x)
    ...
    bw_x_est = bw_x_grid[int(bb1_min[0])]
    bw_prob_est = bw_prob_grid[int(bb2_min[0])]
    bw_prod_est = bw_prod_grid[int(bb3_min[0])]
    lambda_reg_est = lambda_reg_grid[int(ll_min[0])]
